// https://github.com/scoopr/auplay
...
public:
    bool load(CFURLRef url)
    {
        ...
        bool isFormatVBR = ( // 1
            aqData.mDataFormat.mBytesPerPacket == 0 || aqData.mDataFormat.mFramesPerPacket == 0);
...
#if 1
        status = AudioQueuePrime(aqData.mQueue, kNumberBuffers, NULL);
        checkStatus(status);
#endif
    }
    ...
    double duration() const
    {
        ...
        OSStatus     status = AudioFileGetProperty(
            aqData.mAudioFile, kAudioFilePropertyEstimatedDuration, (UInt32*) &sz, &dur);
        checkStatus(status);
        return dur;
    }
    ...
    void seek(double sec)
    {
        ...
        OSStatus     status = AudioFileGetProperty(
            aqData.mAudioFile, kAudioFilePropertyFrameToPacket, (UInt32*) &sz, &trans);

        seekToPacket(trans.mPacket);
        trans.mFrameOffsetInPacket = 0; // Don't support sub packet seeking..

        status = AudioFileGetProperty(
            aqData.mAudioFile, kAudioFilePropertyPacketToFrame, (UInt32*) &sz, &trans);
    ...
    double progress() const
    {
        ...
        OSStatus       status = AudioQueueGetCurrentTime(aqData.mQueue, NULL, &timeStamp, NULL);
...
private:
    ...
    )
    {
        ...
        if ( // 10
            *outBufferSize > maxBufferSize && *outBufferSize > maxPacketSize)
...
AudioPlayer*
AudioPlayer::file(const char* fn)
{
